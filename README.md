...
本仓库为俞烨涛、金施成和刘洁三位同学的NBU机器学习课程报告《基于混合效应模型和双阶段随机森林的胎儿DNA浓度预测与染色体异常检测》的**代码实现**。
...
### ⚠️ 首次运行 MATLAB 脚本前的必要设置

MATLAB 脚本通过项目根目录下的 `config.m` 自动定位数据路径。首次运行时，可能会遇到以下错误：

```text
在当前文件夹或 MATLAB 路径中未找到 config
```

这是因为 MATLAB 当前没有指向项目根目录。**解决方法如下（任选其一）**：

#### 方法一：在 MATLAB 界面中切换当前文件夹

1. 在 MATLAB 主界面上方，找到 **“当前文件夹”** 工具栏。
2. 点击路径栏右侧的浏览按钮 `...`，导航到你解压后的项目根目录（即包含 `config.m` 和 `data/` 文件夹的目录）。
3. 选中该文件夹，点击“选择文件夹”。
4. 此时当前文件夹已切换至项目根目录，再次运行脚本即可。
...
将 `你的项目根目录路径` 替换为你本地实际的项目根目录。你可以先通过文件资源管理器进入项目根目录，复制地址栏中的路径，再粘贴到命令窗口中。

#### 方法三：将项目根目录添加到 MATLAB 路径
...
这样即使当前文件夹不在根目录，MATLAB 也能找到 `config.m`。只需执行一次，后续无需重复操作。

> **提示**：如果当前文件夹位于子目录（如 `src/male_fetal/`）中，MATLAB 无法自动向上查找根目录，因此必须切换或添加路径。

完成上述设置后，在 MATLAB 命令窗口直接输入文件名运行即可。
